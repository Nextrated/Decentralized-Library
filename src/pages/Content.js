...
export default function Content({showAll, showShared, showPublic, showPrivate, showSearchPage, setSearchPage, showAllFilesPage}) {
  ...
  const search = (y) => {
      y= y.toLowerCase();
        const res = allFiles?.filter(x=> x.name.toLowerCase().includes(y) || x.cid.toLowerCase().includes(y));
...
//   }
  ...
  return (
    <Grid minH="100vh" p={3}>
    <Box d="flex" mx="auto" w={{base:"100%", md:"80%", lg:"70%"}} h="auto" mt="100px" px={{base:5 , md:10, lg:12 }}>
        <Input
        type="text"
    ...
    {showSearchPage ? <SearchFiles files={searchedFiles}  showAll={showAllFilesPage}/> : null}
  </Grid>
  )
}
